...
class DataStore:
    ...
    def connect(self):
        ...
        if len(self.conn_status) > 0:
            for index, item in enumerate(self.conn_status):
                if self.conn_status[index] == True: 
                    self.conn_status[index] = False
                    try:
                        self.cur_pool[index] = self.conn_pool[index].cursor()
                    except:
                        print('%s|vd|class DataStore|def connect|connection error when trying to create cursor|%s' % (newDateStamp(),str(sys.exc_info()[1])))
                        self.connect_reset(index)
                    j = index
                    break;
        if j == -1:
            self.conn_status.append(False)
            j = len(self.conn_pool) - 1           
            self.conn_pool.append(mdb.connect(self.info["host"], self.info["user"], self.info["password"], self.info["database"]))
            self.conn_pool[j].autocommit(True)
    ...
    def connect_reset(self,cursor):
        ...
        try:
            ...
            cursorReset = 2
        self.conn_pool[cursor] = mdb.connect(self.info["host"], self.info["user"], self.info["password"], self.info["database"])
...
class FileCacheManager:
    ...
    def addFile(self,fileName,tapeName):
        ...
        if not self.fileTapes.has_key(fileName):
    ...
    def __init__(self):
        ...
        try:
            with open("EchoLeaf.config", "r") as configuration:
                self.info =  json.loads(configuration.readline()) 
            self.info["sysadminpassword"] = decryptIt(self.info["sysadminpassword"])
            self.info["password"] = decryptIt(self.info["password"])           
            self.info["error"] = 0
            self.info["errormsg"] = ''                 
        except:
            self.info["error"] = 2
        ...
        if self.info["error"] == 0:
            try:
                conn = mdb.connect(self.info["host"], self.info["user"], self.info["password"], self.info["database"]);
                cur = conn.cursor()
...
def main():   
    ...
    if configInfo["error"] == 0:
        FUSE(VirtualDrive(configInfo),  configInfo["VDiskMountPoint"], foreground=True, allow_other=True, nothreads=True)
    else:
