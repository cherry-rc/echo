...
# Revised to grow the filemovelog function for later reporting.

def fileMoveLog(report_days=3):  # Added report_days parameter with default 3 days
    ...
    if info["error"] == 0:
        try:
            ...
            # cur.execute("DELETE FROM FileMoveLog WHERE EntryDate < (NOW() - INTERVAL 1 DAY)")  # Removed DELETE statement

            # Modified SQL query to use report_days
            cur.execute(
                f"SELECT EntryDate, Source, Destination, FileName FROM FileMoveLog "
                f"WHERE EntryDate >= (NOW() - INTERVAL {report_days} DAY) "  # Added date filter
                f"ORDER BY EntryDate DESC LIMIT 0, 100")  
...
def fileSearch(searchCriteria):
    ...
    try:
        ...
        # Modified to include "or" condition
        if searchCriteria['file'] or searchCriteria['directory']:
        ...
        """  # Construct the full file path.
        if where_clauses:
            sql += " WHERE " + " OR ".join(where_clauses)  # Use "OR" to combine conditions
